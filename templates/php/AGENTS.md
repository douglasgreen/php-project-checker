...
This document defines the engineering standards, architectural patterns, and quality requirements
for this project. **You must follow these instructions for every code generation task.**
...
- **Strict Typing:** All function parameters and return values must have explicit types. Use `mixed`
  only as a last resort.
- **Modern PHP:** Use PHP 8.3 features where applicable (e.g., `readonly` classes, constructor
  property promotion, DNF types).
- **Immutability:** Default to `final readonly class` unless inheritance or mutation is explicitly
  required.
...
Before suggesting a solution is "complete," ensure it passes the project's QA suite. If you have
terminal access, run:
...
- If modifying `.env` files, only touch `.env.example`. **Never commit or suggest code that modifies
  `.env.local`.**
...
- If adding a major feature, suggest an update to `docs/architecture.md` or create a new ADR
