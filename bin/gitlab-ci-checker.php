...
class GitLabCiChecker
{
    ...
    // Standards version requirements
    private array $requiredStages = ['build', 'test', 'deploy']; // Minimum recommended
    ...
    private function loadCiConfiguration(): void
    {
        ...
        $this->ciConfig = $this->parseYaml($content, '.gitlab-ci.yml');
    ...
    private function parseYaml(string $content, string $filename): ?array
    {
    ...
    private function processInclude(string $path, string $basePath): void
    {
        ...
        if (file_exists($fullPath)) {
            ...
            $parsed = $this->parseYaml($content, $path);
            if ($parsed !== null) {
                $this->includedFiles[$path] = $parsed;
                echo "Loaded included file: {$path}\n";
    ...
    private function collectJobs(): void
    {
        ...
        foreach ($allConfigs as $source => $config) {
            foreach ($config as $key => $value) {
                ...
                if (str_starts_with($key, '.') || in_array($key, ['stages', 'variables', 'workflow', 'include', 'default'])) {
                    continue;
                }
    ...
    private function validateFileStructure(): void
    {
        ...
        if (!file_exists($this->rootDir . '/.gitlab-ci.yml')) {
            $this->addIssue(self::MUST, "Missing CI file", "root", 
                ".gitlab-ci.yml must be at project root per 1.1.1");
        }
        
        // Check for circular includes (1.1.3) - simplified check
        $includePaths = [];
        foreach ($this->includedFiles as $path => $config) {
            if (in_array($path, $includePaths)) {
                $this->addIssue(self::MUST, "Circular include", $path,
        ...
        foreach (array_keys($this->includedFiles) as $path) {
            if (preg_match('/\.gitlab\/ci\/(build|test|deploy|lint|security)\.yml$/', $path)) {
                $hasOrganizedIncludes = true;
            }
        }
        
        if (count($this->includedFiles) > 0 && !$hasOrganizedIncludes) {
            $this->addIssue(self::SHOULD, "Include organization", "include",
    ...
    private function validateYamlSyntax(): void
    {
        ...
        if (preg_match('/^\t+/m', $content)) {
            $this->addIssue(self::MUST, "Tab indentation", ".gitlab-ci.yml",
        ...
        foreach ($lines as $i => $line) {
            if (strlen($line) > $this->config['maxLineLength'] && !str_starts_with(trim($line), '#')) {
                $this->addIssue(self::SHOULD, "Line too long", ".gitlab-ci.yml:" . ($i + 1),
                    "Line exceeds {$this->config['maxLineLength']} characters per 2.4");
            }
        }
        ...
        if (!$hasBlockScalars && preg_match('/script:\s*\["[^"]{100,}"\]/', $content)) {
            $this->addIssue(self::SHOULD, "Inline script", ".gitlab-ci.yml",
    ...
    private function validateStages(): void
    {
        ...
        if (!isset($this->ciConfig['stages'])) {
            $this->addIssue(self::MUST, "Missing stages", "stages",
        ...
        foreach ($expectedOrder as $stage) {
            $pos = array_search($stage, $normalizedStages);
            if ($pos !== false) {
                if ($pos < $lastIndex) {
                    $this->addIssue(self::SHOULD, "Stage ordering", "stages",
        ...
        foreach ($this->allJobs as $name => $job) {
            ...
            if (empty($stage)) {
                $this->addIssue(self::MUST, "Missing stage", "{$name}: stage",
                    "Every job must have explicit stage assignment per 1.2.2");
            } elseif (!in_array($stage, $stages)) {
                $this->addIssue(self::MUST, "Invalid stage", "{$name}: {$stage}",
                    "Stage '{$stage}' not defined in stages list per 1.2.2");
            }
        }
    }
    ...
    private function validateWorkflow(): void
    {
        ...
        if (!isset($this->ciConfig['workflow']['rules'])) {
            $this->addIssue(self::SHOULD, "Missing workflow rules", "workflow",
        ...
        if (preg_match('/^\s*(only|except):\s*$/m', $content)) {
            $this->addIssue(self::MUST, "Deprecated keywords", "control flow",
    ...
    private function validateJobs(): void
    {
        foreach ($this->allJobs as $name => $job) {
            ...
            if (!preg_match('/^[a-z0-9]+(-[a-z0-9]+)*$/', $name)) {
                $this->addIssue(self::MUST, "Invalid job name", $name,
                    "Use kebab-case (e.g., lint:yaml, test:unit) per 1.3.1");
            }
            
            // Check for tags (5.1)
            if (empty($job['config']['tags'])) {
                $this->addIssue(self::MUST, "Missing tags", "{$name}: tags",
                    "Every job must specify tags for runner selection per 5.1");
            }
            
            // Check timeout (4.4)
            if (empty($job['config']['timeout'])) {
                $this->addIssue(self::SHOULD, "Missing timeout", "{$name}: timeout",
                    "Define timeout for long-running jobs per 4.4");
            }
            
            // Check interruptible (5.4, 5.5)
            $isDeployment = str_contains($name, 'deploy') || str_contains($name, 'prod');
            if ($isDeployment) {
                if (!empty($job['config']['interruptible']) && $job['config']['interruptible'] !== false) {
                    $this->addIssue(self::MUST, "Unsafe interruptible", "{$name}: interruptible",
                        "Deployment jobs should have interruptible: false per 5.5");
                }
            } else {
                if ($this->config['requireInterruptible'] && !isset($job['config']['interruptible'])) {
                    $this->addIssue(self::SHOULD, "Missing interruptible", "{$name}: interruptible",
            ...
            if (!isset($job['config']['extends']) && count($job['config']) > 10) {
                $this->addIssue(self::MAY, "Reusable template", $name,
            ...
            if (isset($job['config']['retry'])) {
                ...
                if (is_int($retry) && $retry > 3) {
                    $this->addIssue(self::SHOULD, "Excessive retries", "{$name}: retry",
        ...
        foreach ($this->allJobs as $name => $job) {
            ...
            if ($this->config['requireStrictMode']) {
                ...
                if (!$hasStrictMode && count($allScripts) > 1) {
                    $this->addIssue(self::MUST, "Missing strict mode", "{$name}: script",
            ...
            if (count($allScripts) > 50) {
                $this->addIssue(self::SHOULD, "Long script", "{$name}: script",
            ...
            if (preg_match('/\.\/vendor\/bin\//', $scriptText)) {
                $this->addIssue(self::SHOULD, "Direct vendor/bin usage", "{$name}: script",
            ...
            if (preg_match('/\.\/bin\/\w+\.sh/', $scriptText)) {
                ...
                foreach ($matches[1] as $scriptFile) {
                    ...
                    if (!file_exists($fullPath)) {
                        $this->addIssue(self::MUST, "Missing script file", "{$name}: script",
                            "Referenced script bin/{$scriptFile} not found per file location standards");
                    }
                }
            }
            ...
            foreach ($this->deprecatedComposerFlags as $flag) {
                if (str_contains($scriptText, $flag)) {
                    $this->addIssue(self::MUST, "Deprecated flag", "{$name}: script",
                        "Remove deprecated '{$flag}' from composer commands per user requirements");
                }
            }
            
            // Check for error handling (4.3)
            if (!preg_match('/echo\s*>\&2/', $scriptText) && preg_match('/exit\s+1/', $scriptText)) {
                $this->addIssue(self::MAY, "Error messaging", "{$name}: script",
    ...
    private function validateCaching(): void
    {
        ...
        foreach ($this->allJobs as $name => $job) {
            ...
            if ($this->config['requireUntrackedCache'] && !isset($cache['untracked'])) {
                $this->addIssue(self::MUST, "Missing untracked cache", "{$name}: cache",
            ...
            if (isset($cache['key'])) {
                ...
                if (is_string($key) && !preg_match('/(composer|package)-?lock|yarn\.lock|package-lock\.json/', $key)) {
                    ...
                    if (!isset($cache['key']['files'])) {
                        $this->addIssue(self::MUST, "Unsafe cache key", "{$name}: cache.key",
                        ...
                        if (!$hasLockfile) {
                            $this->addIssue(self::MUST, "Missing lockfile in cache key", "{$name}: cache.key.files",
            ...
            if (isset($cache['policy']) && $cache['policy'] === 'push') {
                // This job updates cache, check if it should
                if (str_contains($name, 'lint') || str_contains($name, 'test')) {
                    $this->addIssue(self::MAY, "Cache push policy", "{$name}: cache.policy",
    ...
    private function validateArtifacts(): void
    {
        foreach ($this->allJobs as $name => $job) {
            ...
            if (empty($artifacts['expire_in']) && !isset($artifacts['expire_in'])) {
                $this->addIssue(self::MUST, "Missing artifact expiry", "{$name}: artifacts",
                    "Set 'expire_in' for all artifacts per 6.2.2");
            } elseif (isset($artifacts['expire_in']) && $artifacts['expire_in'] === 'never') {
                $this->addIssue(self::SHOULD, " Permanent artifacts", "{$name}: artifacts.expire_in",
            ...
            foreach ($paths as $path) {
                foreach ($sensitivePatterns as $pattern) {
                    if (str_contains(strtolower($path), $pattern)) {
                        $this->addIssue(self::MUST, "Sensitive artifact", "{$name}: artifacts.paths",
            ...
            if (isset($artifacts['reports'])) {
                if (isset($artifacts['reports']['junit']) || isset($artifacts['reports']['coverage'])) {
        ...
        if ($this->config['requirePinnedImages']) {
            ...
            foreach ($this->allJobs as $name => $job) {
                ...
                if (str_ends_with($imageName, ':latest') || !str_contains($imageName, ':')) {
                    $this->addIssue(self::MUST, "Unpinned image", "{$name}: image",
                ...
                if (preg_match('/php:(\d+\.\d+)/', $imageName, $matches)) {
                    ...
                    if ($this->composer !== null) {
                        ...
                        if (!empty($cleanComposerPhp) && version_compare($ciPhpVersion, $cleanComposerPhp, '<')) {
                            $this->addIssue(self::MUST, "PHP version mismatch", "{$name}: image",
                                "CI PHP {$ciPhpVersion} < Composer PHP {$cleanComposerPhp} per user requirements");
                        }
                    }
                }
            }
        ...
        if (preg_match_all('/php[:\-]?(\d+\.\d+)/i', $content, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                if (version_compare($foundVersion, $cleanComposerPhp, '<')) {
                    $this->addIssue(self::MUST, "PHP version outdated", "CI configuration",
                        "Found PHP {$foundVersion} reference, but composer.json requires {$cleanComposerPhp} per user requirements");
                }
            }
        }
    }
    ...
    private function validateSecurity(): void
    {
        ...
        foreach ($this->securityPatterns as $pattern => $desc) {
            if (preg_match($pattern, $allContent)) {
                $this->addIssue(self::MUST, "Security violation", "YAML content",
        ...
        if (preg_match('/echo\s+\$[A-Z_]*(?:TOKEN|KEY|SECRET|PWD|PASS)/i', $allContent)) {
            $this->addIssue(self::MUST, "Secret exposure risk", "scripts",
        ...
        foreach ($this->allJobs as $name => $job) {
            if (!empty($job['config']['privileged'])) {
                $this->addIssue(self::MUST, "Privileged container", "{$name}: privileged",
        ...
        foreach ($this->allJobs as $name => $job) {
            ...
            if ($isDeployment) {
                if (empty($job['config']['environment'])) {
                    $this->addIssue(self::MUST, "Missing environment", "{$name}",
                        "Deployment jobs require environment block per 7.4");
                } else {
                    $env = $job['config']['environment'];
                    if (is_array($env)) {
                        if (empty($env['name'])) {
                            $this->addIssue(self::MUST, "Missing environment name", "{$name}: environment",
                                "Environment must have name per 7.4");
                        }
                        if (str_contains(strtolower($env['name']), 'prod') || str_contains($name, 'prod')) {
                            // Check for protection
                            if (empty($job['config']['resource_group'])) {
                                $this->addIssue(self::MUST, "Missing resource group", "{$name}",
                            ...
                            if (!isset($job['config']['environment']['deployment_tier']) || 
                                ...
                                $this->addIssue(self::SHOULD, "Production tier", "{$name}: environment",
                ...
                if (str_contains(strtolower($name), 'prod') && 
                    (!isset($job['config']['rules']) || !preg_match('/when:\s*manual/i', json_encode($job['config'])))) {
                    $this->addIssue(self::SHOULD, "Missing manual gate", "{$name}: rules",
        ...
        foreach ($this->allJobs as $name => $job) {
            if (!isset($job['config']['rules'])) {
                if (!isset($job['config']['only']) && !isset($job['config']['except'])) {
                    $this->addIssue(self::SHOULD, "Missing rules", "{$name}: rules",
            ...
            if (!$hasNeverAtEnd && count($rules) > 1) {
                $this->addIssue(self::MAY, "Rule finalization", "{$name}: rules",
            ...
            foreach ($rules as $rule) {
                $if = $rule['if'] ?? '';
                if (str_contains($if, 'CI_PIPELINE_SOURCE') && str_contains($if, 'merge_request')) {
                    $hasMrRule = true;
                }
            }
            
            if (str_contains($name, 'test') || str_contains($name, 'lint')) {
                if (!$hasMrRule && $this->config['requiredWorkflowRules']) {
                    $this->addIssue(self::MUST, "Missing MR rule", "{$name}: rules",
                        "Test/lint jobs must run on merge requests per 8.1");
                }
            }
            
            // Security check: production deployment restrictions (9.1, 7.6)
            if (str_contains($name, 'prod') || str_contains($name, 'production')) {
                $hasBranchProtection = false;
                foreach ($rules as $rule) {
                    $if = $rule['if'] ?? '';
                    if (str_contains($if, 'CI_COMMIT_BRANCH') || str_contains($if, 'CI_DEFAULT_BRANCH')) {
                        $hasBranchProtection = true;
                    }
                }
                
                if (!$hasBranchProtection) {
                    $this->addIssue(self::MUST, "Unprotected deployment", "{$name}: rules",
        ...
        foreach ($this->allJobs as $name => $job) {
            ...
            if (is_string($env)) {
                ...
                if (!preg_match('/^[a-z0-9-]+$/', $env)) {
                    $this->addIssue(self::SHOULD, "Environment naming", "{$name}: environment",
                        "Use lowercase with hyphens per naming conventions");
                }
            } elseif (is_array($env)) {
                // Check for URL
                if (empty($env['url'])) {
                    $this->addIssue(self::SHOULD, "Missing environment URL", "{$name}: environment",
    ...
    private function printReport(): void
    {
        ...
        $deduction = (count($mustIssues) * 5) + (count($shouldIssues) * 1);
