...
class PackageJsonChecker
{
    ...
    private function validateDependencies(): void
    {
        ...
        foreach ($allDeps as $pkg) {
            ...
            if (($inDeps && $inDev) || ($inDeps && $inPeer) || ($inDev && $inPeer)) {
                ...
                if ($inDev) {
                    $versions[] = $devDeps[$pkg];
                }

                if ($inPeer) {
                    $versions[] = $peerDeps[$pkg];
                }
                ...
                if (count($unique) > 1) {
                    $this->addIssue(
                        ...
                        'Version mismatch',
                        $pkg,
                        sprintf("Package '%s' has different versions: ", $pkg) . implode(', ', $versions),
                    );
                }
            }
        }
    ...
    private function validateScripts(): void
    {
        ...
        foreach ($scripts as $name => $command) {
            ...
            if (str_contains((string) $name, 'lint')) {
                $hasLint = true;
            }

            if ($name === 'test' || str_contains((string) $name, 'test:')) {
                $hasTest = true;
            }

            if (str_contains((string) $name, 'format')) {
                $hasFormat = true;
            }
            ...
            if (is_string($command)) {
                ...
                if (preg_match('/\.\.\/|^\.\/|^\//', $command) && !str_contains($command, 'node_modules')) {
                    $this->addIssue(
                        ...
                        'scripts.' . $name,
                        sprintf("Avoid hardcoded paths in scripts: '%s'", $command),
                    );
                }
    ...
    private function validateCrossFileConsistency(): void
    {
        ...
        if ($pkgKebab !== $composerKebab && !empty($pkgProject) && !empty($composerProject)) {
            $this->addIssue(
                ...
                'Name mismatch',
                'name',
                sprintf("package.json name '%s' differs from composer.json '%s'", $pkgProject, $composerProject),
            );
        }
        ...
        if (!empty($pkgLicense) && !empty($composerLicense) &&
            ...
            $this->addIssue(
                ...
                'License mismatch',
                'license',
                sprintf("License '%s' differs from composer.json '%s'", $pkgLicense, $composerLicense),
            );
        }
    }
    ...
    private function validatePublicFields(): void
    {
        ...
        if (empty($homepage)) {
            ...
            if (!str_starts_with((string) $homepage, $expectedPattern)) {
                $this->addIssue(
                    self::MUST,
                    'Invalid homepage',
                    'homepage: ' . $homepage,
                    sprintf('Must match: https://github.com/%s/<project>', $this->config['owner']),
                );
            }
        }
        ...
        if (empty($repo['url'])) {
            ...
            if (!str_contains((string) $repo['url'], $expectedPattern) && !str_contains((string) $repo['url'], 'github.com')) {
                $this->addIssue(
                    self::SHOULD,
                    'Repository mismatch',
                    'repository.url',
        ...
        if (!empty($bugs['url'])) {
            $expectedPattern = 'github.com/' . $this->config['owner'] . '/';
            if (!str_contains((string) $bugs['url'], $expectedPattern)) {
                $this->addIssue(
                    self::SHOULD,
                    'Bugs URL mismatch',
                    'bugs.url',
